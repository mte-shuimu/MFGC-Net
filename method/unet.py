...
class UNet(nn.Module):
    ...
    ## U-Net
    """
    def __init__(self, in_channels: int, n_classes: dict):
        """
        ...
        self.sic_final_conv = nn.Conv2d(64, n_classes['SIC'], kernel_size=1)
        self.sod_final_conv = nn.Conv2d(64, n_classes['SOD'], kernel_size=1)
        self.floe_final_conv = nn.Conv2d(64, n_classes['FLOE'], kernel_size=1)
    ...
    def forward(self, x: torch.Tensor):
        ...
        sic_output = self.sic_final_conv(x)
        sod_output = self.sod_final_conv(x)
        floe_output = self.floe_final_conv(x)


        return {'SIC':sic_output, 'SOD':sod_output, 'FLOE':floe_output}